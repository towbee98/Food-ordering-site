...
router.get('/orders?startdate=1900-01-01&enddate=1900-01-01', function(req, res, next) {
	sess = req.session;
	var startdt = "'"+req.query.startdate+"'"; 
	var enddt =  "'"+req.query.enddate+"'";  
	console.log(startdt)
	console.log(enddt)
    ...
    var restaurant = sess.restaurant;
    if (req.query.startdate.toString().trim()=="1900-01-01" && req.query.enddate.toString().trim()=="1900-01-01" )
    {
    console.log("Empty Start and end date.................")
	...
	if (req.query.startdate.toString().trim()!="1900-01-01" && req.query.enddate.toString().trim()!="1900-01-01")
	{
    console.log("select * from RESTUARANT_MENUs_ORDERS orders JOIN RESTUARANT_MENUS menu ON orders.MENU = menu.RESTUARANT_MENU_ID AND menu.RESTUARANT_ID = %s   and orders.ORDERDATE between %s and %s ",restaurant, startdt, enddt)

    
    connection.query("select * from RESTUARANT_MENUs_ORDERS orders JOIN RESTUARANT_MENUS menu ON orders.MENU = menu.RESTUARANT_MENU_ID AND menu.RESTUARANT_ID = ?   and orders.ORDERDATE between ? and ? ",[restaurant, startdt, enddt], function(err, rows) {
		if(err)
                console.log("Error Selecting : %s ",err );
        console.log(rows);
        res.render('orders',{page_title:"Orders",data:rows});
...
router.post('/login', function(req, res, next) {
    ...
    connection.query("SELECT * FROM RESTUARANT WHERE username = ? AND password = ?", [username, password], function(err, rows) {
        ...
        if (rows.length > 0) {
            ...
            res.redirect('/restaurant/orders?startdate=1900-01-01&enddate=1900-01-01');
        } else {
            var invalid = true
            res.render('restaurant_login', {
                page_title: "Login",
